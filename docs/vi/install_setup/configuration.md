...
```
nơi `/path/to/config.cfg` là đường dẫn đến tệp cấu hình trong hệ thống tệp của máy chủ của bạn (phía bên phải tham chiếu đến đường dẫn trong container và không được thay đổi).
...
Lưu ý rằng các tùy chọn cấu hình được đặt qua môi trường có ưu tiên hơn các tùy chọn trong tệp cấu hình. Nếu cả hai đều có mặt, biến môi trường sẽ "thắng".
...
Khóa | Mô tả
----|-------------
`TREE` | Tên của cơ sở dữ liệu cây gia đình để sử dụng. Hiển thị các cây có sẵn với `gramps -l`. Nếu một cây với tên này không tồn tại, một cây mới trống sẽ được tạo.
...
!!! info
    Bạn có thể tạo một khóa bí mật an toàn ví dụ như với lệnh
...
Khóa | Mô tả
----|-------------
`MEDIA_BASE_DIR` | Đường dẫn để sử dụng làm thư mục gốc cho các tệp phương tiện, ghi đè thư mục gốc phương tiện được đặt trong Gramps. Khi sử dụng [S3](s3.md), phải có dạng `s3://<bucket_name>`
...
`BASE_URL` | URL cơ bản nơi API có thể được truy cập (ví dụ: `https://mygramps.mydomain.com/`). Điều này là cần thiết ví dụ như để xây dựng các liên kết đặt lại mật khẩu chính xác
`CORS_ORIGINS` | Các nguồn nơi yêu cầu CORS được phép. Theo mặc định, tất cả đều bị cấm. Sử dụng `"*"` để cho phép yêu cầu từ bất kỳ miền nào.
...
`EMAIL_USE_TLS` | **Đã ngừng sử dụng** (sử dụng `EMAIL_USE_SSL` hoặc `EMAIL_USE_STARTTLS` thay thế). Boolean, có sử dụng TLS để gửi email hay không. Mặc định là `True`. Khi sử dụng STARTTLS, đặt giá trị này thành `False` và sử dụng cổng khác với 25.
`EMAIL_USE_SSL` | Boolean, có sử dụng SSL/TLS ngầm định cho SMTP (v3.6.0+). Mặc định là `True` nếu `EMAIL_USE_TLS` không được đặt rõ ràng. Thường được sử dụng với cổng 465.
...
`THUMBNAIL_CACHE_CONFIG` | Từ điển với các cài đặt cho bộ nhớ cache hình thu nhỏ. Xem [Flask-Caching](https://flask-caching.readthedocs.io/en/latest/) để biết các cài đặt có thể có.
`REQUEST_CACHE_CONFIG` | Từ điển với các cài đặt cho bộ nhớ cache yêu cầu. Xem [Flask-Caching](https://flask-caching.readthedocs.io/en/latest/) để biết các cài đặt có thể có.
`PERSISTENT_CACHE_CONFIG` | Từ điển với các cài đặt cho bộ nhớ cache bền vững, được sử dụng ví dụ như cho telemetry. Xem [Flask-Caching](https://flask-caching.readthedocs.io/en/latest/) để biết các cài đặt có thể có.
`CELERY_CONFIG` | Các cài đặt cho hàng đợi tác vụ nền Celery. Xem [Celery](https://docs.celeryq.dev/en/stable/userguide/configuration.html) để biết các cài đặt có thể có.
`REPORT_DIR` | Thư mục tạm thời nơi đầu ra của việc chạy các báo cáo Gramps sẽ được lưu trữ
...
`DISABLE_TELEMETRY` | Nếu `True`, vô hiệu hóa thống kê telemetry (mặc định `False`). Xem [telemetry](telemetry.md) để biết chi tiết.
...
Các cài đặt này cần thiết nếu bạn muốn sử dụng các tính năng hỗ trợ AI như trò chuyện hoặc tìm kiếm ngữ nghĩa.

Khóa | Mô tả
----|-------------
`LLM_BASE_URL` | URL cơ bản cho API trò chuyện tương thích với OpenAI. Mặc định là `None`, sử dụng API OpenAI.
