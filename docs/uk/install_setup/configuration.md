...
Використовуючи стандартний образ Docker, всі необхідні налаштування можна зробити з браузера. Однак, в залежності від розгортання, може виникнути необхідність налаштувати конфігурацію сервера.
...
- префіксуйте кожну назву налаштування з `GRAMPSWEB_`, щоб отримати назву змінної середовища
...
Ключ | Опис
----|-------------
`MEDIA_BASE_DIR` | Шлях, який слід використовувати як базовий каталог для медіафайлів, перекриваючи базовий каталог медіа, встановлений у Gramps. При використанні [S3](s3.md) повинен мати форму `s3://<bucket_name>`
...
`BASE_URL` | Базовий URL, за яким можна отримати доступ до API (наприклад, `https://mygramps.mydomain.com/`). Це необхідно, наприклад, для побудови правильних посилань для скидання пароля
...
`EMAIL_USE_TLS` | **Застаріло** (використовуйте `EMAIL_USE_SSL` або `EMAIL_USE_STARTTLS` замість цього). Булеве значення, чи використовувати TLS для надсилання електронних листів. За замовчуванням `True`. При використанні STARTTLS, встановіть це в `False` і використовуйте порт, відмінний від 25.
...
`PERSISTENT_CACHE_CONFIG` | Словник з налаштуваннями для постійного кешу, що використовується, наприклад, для телеметрії. Дивіться [Flask-Caching](https://flask-caching.readthedocs.io/en/latest/) для можливих налаштувань.
...
`REPORT_DIR` | Тимчасова директорія, де буде зберігатися вихід виконання звітів Gramps
`EXPORT_DIR` | Тимчасова директорія, де буде зберігатися вихід експорту бази даних Gramps
...
`PILLOW_MAX_IMAGE_PIXELS` | Встановлює параметр PIL.Image.MAX_IMAGE_PIXELS, який вказує на кількість пікселів, які може містити оброблене зображення. Дивіться [документацію](https://pillow.readthedocs.io/en/stable/reference/Image.html#PIL.Image.MAX_IMAGE_PIXELS) для деталей.


!!! info
    При використанні змінних середовища для конфігурації булеві параметри, такі як `EMAIL_USE_TLS`, повинні бути або рядком `true`, або `false` (чутливі до регістру!).
...
`OIDC_CLIENT_ID` | OAuth ID клієнта (для власних постачальників OIDC)
`OIDC_CLIENT_SECRET` | OAuth секрет клієнта (для власних постачальників OIDC)
`OIDC_NAME` | Власна назва для постачальника. За замовчуванням "OIDC"
...
`OIDC_OPENID_CONFIG_URL` | Необов'язково: URL до кінцевої точки конфігурації OpenID Connect (якщо не використовується стандартний `/.well-known/openid-configuration`)
...
`OIDC_GROUP_ADMIN` | Назва групи/ролі з вашого постачальника OIDC, яка відповідає ролі "Admin" Gramps
`OIDC_GROUP_OWNER` | Назва групи/ролі з вашого постачальника OIDC, яка відповідає ролі "Owner" Gramps
`OIDC_GROUP_EDITOR` | Назва групи/ролі з вашого постачальника OIDC, яка відповідає ролі "Editor" Gramps
`OIDC_GROUP_CONTRIBUTOR` | Назва групи/ролі з вашого постачальника OIDC, яка відповідає ролі "Contributor" Gramps
`OIDC_GROUP_MEMBER` | Назва групи/ролі з вашого постачальника OIDC, яка відповідає ролі "Member" Gramps
`OIDC_GROUP_GUEST` | Назва групи/ролі з вашого постачальника OIDC, яка відповідає ролі "Guest" Gramps
...
`LLM_MODEL` | Модель, яку слід використовувати для API чату, сумісного з OpenAI. Якщо не встановлено (за замовчуванням), чат вимкнено. Починаючи з v3.6.0, AI асистент використовує Pydantic AI з можливостями виклику інструментів.
`VECTOR_EMBEDDING_MODEL` | Модель [Sentence Transformers](https://sbert.net/) для семантичного пошуку векторних вкладів. Якщо не встановлено (за замовчуванням), семантичний пошук і чат вимкнені.
