...
`EMAIL_USE_SSL` | Boolean, ob implizites SSL/TLS für SMTP (v3.6.0+) verwendet werden soll. Standardmäßig `True`, wenn `EMAIL_USE_TLS` nicht ausdrücklich festgelegt ist. Typischerweise mit Port 465 verwendet.
`EMAIL_USE_STARTTLS` | Boolean, ob explizites STARTTLS für SMTP (v3.6.0+) verwendet werden soll. Standardmäßig `False`. Typischerweise mit Port 587 oder 25 verwendet.
...
!!! info
    Bei der Verwendung von Umgebungsvariablen für die Konfiguration müssen boolesche Optionen wie `EMAIL_USE_TLS` entweder den String `true` oder `false` (groß-/kleinschreibungssensitiv!) haben.
...
Schlüssel | Beschreibung
----|-------------
`MEDIA_PREFIX_TREE` | Boolean, ob ein separates Unterverzeichnis für die Mediendateien jedes Baumes verwendet werden soll oder nicht. Standardmäßig `False`, aber es wird dringend empfohlen, in einem Multi-Tree-Setup `True` zu verwenden.
...
`OIDC_SCOPES` | OAuth-Bereiche. Standardmäßig "openid email profile"
...
`VECTOR_EMBEDDING_MODEL` | Das [Sentence Transformers](https://sbert.net/) Modell, das für semantische Suchvektor-Einbettungen verwendet werden soll. Wenn nicht festgelegt (Standard), sind die semantische Suche und der Chat deaktiviert.
`LLM_MAX_CONTEXT_LENGTH` | Zeichenlimit für den Familienstammbaumkontext, der dem LLM bereitgestellt wird. Standardmäßig 50000.
`LLM_SYSTEM_PROMPT` | Benutzerdefinierter System-Prompt für den LLM-Chat-Assistenten (v3.6.0+). Wenn nicht festgelegt, wird der Standard-Prompt für Genealogie verwendet.
