...
Observe que as opções de configuração definidas via ambiente têm precedência sobre as opções no arquivo de configuração. Se ambas estiverem presentes, a variável de ambiente "vence".
...
!!! info
    Ao usar variáveis de ambiente para configuração, opções booleanas como `EMAIL_USE_TLS` devem ser a string `true` ou `false` (sensível a maiúsculas!).


### Configurações apenas para banco de dados backend PostgreSQL

Isso é necessário se você configurou seu banco de dados Gramps para funcionar com o [complemento PostgreSQL](https://gramps-project.org/wiki/index.php/Addon:PostgreSQL).
...
`NEW_DB_BACKEND` | O backend do banco de dados a ser usado para árvores genealógicas recém-criadas. Deve ser um dos `sqlite`, `postgresql` ou `sharedpostgresql`. O padrão é `sqlite`.
...
Essas configurações são necessárias se você quiser usar autenticação OpenID Connect (OIDC) com provedores externos. Para instruções detalhadas de configuração e exemplos, veja [Autenticação OIDC](oidc.md).
...
`OIDC_OPENID_CONFIG_URL` | Opcional: URL para o endpoint de configuração do OpenID Connect (se não estiver usando o padrão `/.well-known/openid-configuration`)
...
Essas configurações são necessárias se você quiser usar recursos impulsionados por IA, como chat ou busca semântica.
