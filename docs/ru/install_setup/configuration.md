...
Ключ | Описание
----|-------------
`MEDIA_BASE_DIR` | Путь, который будет использоваться в качестве базового каталога для медиафайлов, переопределяющий базовый каталог медиафайлов, установленный в Gramps. При использовании [S3](s3.md) должен иметь вид `s3://<bucket_name>`
...
`DISABLE_TELEMETRY` | Если `True`, отключает статистическую телеметрию (по умолчанию `False`). См. [телеметрия](telemetry.md) для получения подробной информации.
