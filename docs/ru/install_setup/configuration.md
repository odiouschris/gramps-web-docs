...
Ключ | Описание
----|-------------
`TREE` | Имя базы данных семейного дерева для использования. Показать доступные деревья с помощью `gramps -l`. Если дерева с таким именем не существует, будет создано новое пустое.
...
Ключ | Описание
----|-------------
`MEDIA_BASE_DIR` | Путь, который будет использоваться в качестве базового каталога для медиафайлов, переопределяющий базовый каталог медиа, установленный в Gramps. При использовании [S3](s3.md) должен иметь форму `s3://<bucket_name>`
`SEARCH_INDEX_DB_URI` | URL базы данных для индекса поиска. Допускаются только `sqlite` или `postgresql` в качестве бэкендов. По умолчанию `sqlite:///indexdir/search_index.db`, создавая файл SQLite в папке `indexdir`, относительно пути, откуда выполняется скрипт
`STATIC_PATH` | Путь для обслуживания статических файлов (например, статического веб-фронтенда)
...
`EMAIL_USE_TLS` | **Устарело** (используйте `EMAIL_USE_SSL` или `EMAIL_USE_STARTTLS` вместо этого). Булевое значение, указывающее, следует ли использовать TLS для отправки писем. По умолчанию `True`. При использовании STARTTLS установите это значение в `False` и используйте порт, отличный от 25.
`EMAIL_USE_SSL` | Булевое значение, указывающее, следует ли использовать неявный SSL/TLS для SMTP (v3.6.0+). По умолчанию `True`, если `EMAIL_USE_TLS` не установлен явно. Обычно используется с портом 465.
`EMAIL_USE_STARTTLS` | Булевое значение, указывающее, следует ли использовать явный STARTTLS для SMTP (v3.6.0+). По умолчанию `False`. Обычно используется с портом 587 или 25.
...
`DISABLE_TELEMETRY` | Если `True`, отключает телеметрию статистики (по умолчанию `False`). См. [телеметрия](telemetry.md) для получения подробной информации.
...
Ключ | Описание
----|-------------
`MEDIA_PREFIX_TREE` | Булевое значение, указывающее, следует ли использовать отдельную подпапку для медиафайлов каждого дерева. По умолчанию `False`, но настоятельно рекомендуется использовать `True` в настройках с несколькими деревьями
...
Эти настройки необходимы, если вы хотите использовать аутентификацию OpenID Connect (OIDC) с внешними провайдерами. Для подробных инструкций по настройке и примеров смотрите [Аутентификация OIDC](oidc.md).

Ключ | Описание
----|-------------
`OIDC_ENABLED` | Булевое значение, указывающее, следует ли включить аутентификацию OIDC. По умолчанию `False`.
...
`OIDC_NAME` | Пользовательское имя для отображения провайдера. По умолчанию "OIDC"
...
`OIDC_DISABLE_LOCAL_AUTH` | Булевое значение, указывающее, следует ли отключить локальную аутентификацию по имени пользователя/паролю. По умолчанию `False`
`OIDC_AUTO_REDIRECT` | Булевое значение, указывающее, следует ли автоматически перенаправлять на OIDC, когда настроен только один провайдер. По умолчанию `False`
...
Эти настройки позволяют вам сопоставить группы/роли OIDC из вашего провайдера идентификации с ролями пользователей Gramps Web:
...
`VECTOR_EMBEDDING_MODEL` | Модель [Sentence Transformers](https://sbert.net/) для семантического поиска векторных встраиваний. Если не установлено (по умолчанию), семантический поиск и чат отключены.
