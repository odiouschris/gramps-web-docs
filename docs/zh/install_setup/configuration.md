...
`EMAIL_USE_TLS` | **已弃用**（请使用 `EMAIL_USE_SSL` 或 `EMAIL_USE_STARTTLS`）。布尔值，指示是否使用 TLS 发送电子邮件。默认为 `True`。使用 STARTTLS 时，将其设置为 `False` 并使用与 25 不同的端口。
`EMAIL_USE_SSL` | 布尔值，指示是否对 SMTP 使用隐式 SSL/TLS（v3.6.0+）。如果未显式设置 `EMAIL_USE_TLS`，默认为 `True`。通常与端口 465 一起使用。
`EMAIL_USE_STARTTLS` | 布尔值，指示是否对 SMTP 使用显式 STARTTLS（v3.6.0+）。默认为 `False`。通常与端口 587 或 25 一起使用。
...
`PILLOW_MAX_IMAGE_PIXELS` | 设置 PIL.Image.MAX_IMAGE_PIXELS 参数，指示处理的图像可以包含的像素数量。有关详细信息，请参见 [文档](https://pillow.readthedocs.io/en/stable/reference/Image.html#PIL.Image.MAX_IMAGE_PIXELS)。
...
键 | 描述
----|-------------
`MEDIA_PREFIX_TREE` | 布尔值，指示是否为每棵树的媒体文件使用单独的子文件夹。默认为 `False`，但在多树设置中强烈建议使用 `True`
`NEW_DB_BACKEND` | 新创建的家谱树要使用的数据库后端。必须是 `sqlite`、`postgresql` 或 `sharedpostgresql` 之一。默认为 `sqlite`。
`POSTGRES_HOST` | 使用 SharedPostgreSQL 后端时，用于创建新树的 PostgreSQL 服务器的主机名
`POSTGRES_PORT` | 使用 SharedPostgreSQL 后端时，用于创建新树的 PostgreSQL 服务器的端口
...
键 | 描述
----|-------------
`OIDC_ENABLED` | 布尔值，指示是否启用 OIDC 认证。默认为 `False`。
...
`OIDC_DISABLE_LOCAL_AUTH` | 布尔值，指示是否禁用本地用户名/密码认证。默认为 `False`
`OIDC_AUTO_REDIRECT` | 布尔值，指示是否在仅配置一个提供者时自动重定向到 OIDC。默认为 `False`
...
`OIDC_GROUP_ADMIN` | 映射到 Gramps "Admin" 角色的 OIDC 提供者中的组/角色名称
`OIDC_GROUP_OWNER` | 映射到 Gramps "Owner" 角色的 OIDC 提供者中的组/角色名称
`OIDC_GROUP_EDITOR` | 映射到 Gramps "Editor" 角色的 OIDC 提供者中的组/角色名称
`OIDC_GROUP_CONTRIBUTOR` | 映射到 Gramps "Contributor" 角色的 OIDC 提供者中的组/角色名称
`OIDC_GROUP_MEMBER` | 映射到 Gramps "Member" 角色的 OIDC 提供者中的组/角色名称
`OIDC_GROUP_GUEST` | 映射到 Gramps "Guest" 角色的 OIDC 提供者中的组/角色名称
