...
Quando utilizzi la [configurazione basata su Docker Compose](deployment.md), puoi includere un file di configurazione aggiungendo il seguente elemento della lista sotto la chiave `volumes:` nel blocco `grampsweb:`:
...
`STATIC_PATH` | Percorso da cui servire i file statici (ad es. un frontend web statico)
`BASE_URL` | URL di base dove l'API può essere raggiunta (ad es. `https://mygramps.mydomain.com/`). Questo è necessario ad esempio per costruire link corretti per il ripristino della password
...
`EMAIL_HOST` | Host del server SMTP (ad es. per l'invio di e-mail di ripristino della password)
...
`EMAIL_USE_TLS` | **Deprecato** (usa `EMAIL_USE_SSL` o `EMAIL_USE_STARTTLS` invece). Booleano, se utilizzare TLS per l'invio di e-mail. Predefinito a `True`. Quando si utilizza STARTTLS, impostalo su `False` e utilizza una porta diversa da 25.
`EMAIL_USE_SSL` | Booleano, se utilizzare SSL/TLS implicito per SMTP (v3.6.0+). Predefinito a `True` se `EMAIL_USE_TLS` non è impostato esplicitamente. Tipicamente utilizzato con la porta 465.
`EMAIL_USE_STARTTLS` | Booleano, se utilizzare STARTTLS esplicito per SMTP (v3.6.0+). Predefinito a `False`. Tipicamente utilizzato con la porta 587 o 25.
...
Questo è necessario se hai configurato il tuo database Gramps per lavorare con l'[addon PostgreSQL](https://gramps-project.org/wiki/index.php/Addon:PostgreSQL).
...
Chiave | Descrizione
----|-------------
`MEDIA_PREFIX_TREE` | Booleano, se utilizzare o meno una sottocartella separata per i file multimediali di ciascun albero. Predefinito a `False`, ma si raccomanda vivamente di utilizzare `True` in una configurazione multi-albero
...
`OIDC_OPENID_CONFIG_URL` | Opzionale: URL per il punto di configurazione OpenID Connect (se non si utilizza il standard `/.well-known/openid-configuration`)
...
`LLM_MAX_CONTEXT_LENGTH` | Limite di caratteri per il contesto dell'albero genealogico fornito all'LLM. Predefinito a 50000.
